...
class SoftPty extends EventEmitter {
   ...
   * @param {boolean} [opts.handleFlowControl=false] - node-pty compat:
   *   when true, write() intercepts the pause/resume control bytes below
   *   instead of forwarding them to the child, and calls pause()/resume()
   *   on our own output streams. Mirrors node-pty's documented flow
   *   control behavior (used e.g. by xterm.js for backpressure).
   * @param {string} [opts.flowControlPause='\x13'] - DC3/XOFF by default
   * @param {string} [opts.flowControlResume='\x11'] - DC1/XON by default
   * @param {number} [opts.uid] - passed straight through to child_process.spawn
   * @param {number} [opts.gid] - passed straight through to child_process.spawn
   * @param {string} [opts.argv0] - passed straight through to child_process.spawn
   */
  constructor(file, args = [], opts = {}) {
    ...
    this.handleFlowControl = !!opts.handleFlowControl;
    this._flowControlPause = opts.flowControlPause || '\x13';
    this._flowControlResume = opts.flowControlResume || '\x11';
   ...
   * than letting a lost write take down the caller. If handleFlowControl
   * is on, the pause/resume control bytes are intercepted here (matching
   * node-pty) rather than forwarded to the child. */
  write(data) {
    if (this._exited) return;
    if (this.handleFlowControl) {
      if (data === this._flowControlPause) { this.pause(); return; }
  ...
  /** node-pty compat: pause the underlying output streams. Real
   * backpressure (not cosmetic) — Node stops delivering 'data' events
   * and buffers internally until resume() is called. */
  pause() {
    this._child.stdout.pause();
    this._child.stderr.pause();
  }

  /** node-pty compat: resume output streams paused via pause(). */
  resume() {
    this._child.stdout.resume();
    this._child.stderr.resume();
  }

  /** node-pty compat no-op. Real node-pty clears the terminal's own
   * scrollback buffer via an escape sequence; there is no such buffer
   * here (this class only ever emits raw process output), so there is
   * nothing to clear. Present so code written against IPty doesn't
   * throw calling it. */
  clear() {}
